...
### [Post-build] — Async resume/cover-letter generation with status polling
**What:** Changed `POST /jobs/{id}/resume` and autonomous `POST /jobs/{id}/cover-letter` to fire-and-forget (`InvocationType='Event'`), returning 202 immediately. Both Lambdas now write `{prefix}_status` (`generating`/`done`/`failed`), `{prefix}_started_at`, and `{prefix}_error` to DynamoDB at the start, on success, and on failure. A new `GET /jobs/{id}/generation-status` endpoint returns the current status for both resume and cover letter, including fresh 24-hour presigned S3 URLs when done. `GET /jobs/{id}` also now includes these status objects in its response. A staleness check in the API converts any `generating` status older than 5 minutes to `failed` to handle silent Lambda timeouts.
**Why:** API Gateway has a hard 29-second response timeout; resume-tailor takes ~30 seconds due to two LLM passes + PDF render + S3 upload. Synchronous invocation was hitting this timeout and producing a CORS-flavored error on the dashboard.
**Alternatives considered:** Lambda Function URLs with longer timeouts (avoids API Gateway but adds a separate endpoint and auth surface); Step Functions with callback (heavyweight for a single-step task); SQS-based queue (adds more infrastructure for no gain at single-user scale).
